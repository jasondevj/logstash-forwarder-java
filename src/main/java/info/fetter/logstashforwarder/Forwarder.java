...
import info.fetter.logstashforwarder.config.NetworkSection;
import info.fetter.logstashforwarder.protocol.TcpClient;
import info.fetter.logstashforwarder.protocol.UdpClient;
...
public class Forwarder {
    ...
    private static void infiniteLoop() throws IOException, InterruptedException {
        while (true) {
            try {
                ...
                logger.error("Lost server connection", e);
    ...
    private static void connectToServer() {
        ...
        List<NetworkSection.Server> serverList = configManager.getConfig().getNetwork().getServers();
        ...
        while (adapter == null) {
            NetworkSection.Server server = serverList.get(randomServerIndex);
            randomServerIndex = random.nextInt(serverList.size());
            try {
                String[] serverAndPort = server.getConnection().split(":");
                logger.info("Trying to connect to " + server);
                if (server.getProtocol().equalsIgnoreCase("udp")) {
                    adapter = new UdpClient(serverAndPort[0], Integer.parseInt(serverAndPort[1]), networkTimeout);
                } else if (server.getProtocol().equalsIgnoreCase("tcp")) {
                    adapter = new TcpClient(serverAndPort[0], Integer.parseInt(serverAndPort[1]), networkTimeout);
                } else {
                    logger.error("Unknown network connection type [" + server.getProtocol() + "]");
                }
                fileReader.setAdapter(adapter);
                inputReader.setAdapter(adapter);
            } catch (Exception ex) {
                if (logger.isDebugEnabled()) {
                    logger.error("Failed to connect to server " + server + " : ", ex);
                } else {
                    logger.error("Failed to connect to server " + server + " : " + ex.getMessage());
                }
                try {
                    Thread.sleep(networkTimeout);
